...
import { Bot, History, Inbox, Settings } from "lucide-react";
import { Link, useLocation } from "wouter";

/*
 * AppLayout - Main application shell
 * 
 * Design: Light sidebar with Milo branding
 * Theme: White/light matching platform.get-milo.com
 * Layout: Sidebar navigation (better enterprise UX)
 */
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-background">
      ...
      <aside className="w-16 bg-sidebar border-r border-sidebar-border flex flex-col items-center py-4">
        {/* Logo */}
        <Link href="/">
          <div className="w-10 h-10 rounded-lg border border-border flex items-center justify-center mb-6 hover:bg-accent transition-colors">
            ...
            return (
              <Link key={item.href} href={item.href}>
                ...
                >
                  ...
                  {item.badge && (
                    <span className="absolute -top-1 -right-1 w-4 h-4 rounded-full bg-[var(--color-status-pending)] text-[10px] font-medium text-white flex items-center justify-center">
                      {item.badge}
                    </span>
                  )}
                </button>
